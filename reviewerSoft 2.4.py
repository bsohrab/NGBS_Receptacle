...
def dissect(file):
    ...
    # #fill = PatternFill(start_color='538DD5', end_color='538DD5', fill_type='solid')
    #
    # psheet.conditional_formatting.add('A1:F104', CellIsRule(operator='containsText', formula=['0'], fill=fill))

    # cellref = 'A1:F60'  # adjusted to show more cells
    # red_font = Font(size=14, bold=True, color='ffffff')
    # red_fill = PatternFill(start_color='ffcccc', end_color='ffcccc', fill_type='solid')
    #
    # psheet.conditional_formatting.add(cellref, Rule(type='containsText', operator='containsText', formula=["=\"rough\""],dxf=DifferentialStyle(fill=red_fill, font=red_font)))
    ...
    psheet.conditional_formatting.add('$D$1:$D175',FormulaRule(formula=['=ISNUMBER(SEARCH("Rough/Final",D1))'], stopIfTrue=False,fill=fill1))
    psheet.conditional_formatting.add('$D$1:$D175',FormulaRule(formula=['=ISNUMBER(SEARCH("Final",D1))'], stopIfTrue=False,fill=fill2))
    psheet.conditional_formatting.add('$D$1:$D175',FormulaRule(formula=['=ISNUMBER(SEARCH("Rough",D1))'], stopIfTrue=False,fill=fill3))

    #psheet.conditional_formatting.add('A1:F100', rule)
    # redFill = PatternFill(start_color='EE1111',
    #                       end_color='EE1111',
    #                       fill_type='solid')
    #
    # ws.conditional_formatting.add('G2:G339',
    #                               CellIsRule(operator='notEqual', formula=['$F2'] and ['$H2'], stopIfTrue=True,
    #                                          fill=redFill))
