...
def retrieveoverview(worksheetOV):
    ...
    for row_num in range(1,worksheetOV.max_row):
        for col_num in range(1,worksheetOV.max_column):
            ...
            if worksheetOV.cell(row_num, col_num).value is not None:
                # print(worksheetOV.cell(row_num, col_num).value)
                if "Address" in str(worksheetOV.cell(row_num, col_num).value):
                    address = worksheetOV.cell(row_num, col_num + 4).value
                    #Community/Lot #
                    if worksheetOV.cell(row_num+1, col_num+4).value is None:
                        lot = " "
                    else:
                        lot = worksheetOV.cell(row_num+1, col_num+4).value
                    # City
                    if worksheetOV.cell(row_num + 2, col_num + 4).value is None:
                        city = " "
                    else:
                        city = worksheetOV.cell(row_num+2, col_num + 4).value
                    # State
                    if worksheetOV.cell(row_num + 3, col_num + 4).value is None:
                        state = " "
                    else:
                       state = worksheetOV.cell(row_num + 3, col_num + 4).value
                   # County
                   # if worksheetOV.cell(row_num + 4, col_num + 4).value is None:
                   #     cou = " "
                   # else:
                   #     cou = worksheetOV.cell(row_num + 4, col_num + 4).value

                elif address != 0:
                    return str(address)+" "+str(lot)+ " " + str(city) + " " +str(state)
...
def dissect(file):
    ...
    xlname = retrieveoverview(worksheetOV)
    print(xlname)
        ...
        code = '''        
                ...
                For Each ws In xlBook.Worksheets
                    ...
                    Select Case mySheetName
                        ...
                        'xlSheet.Range("Z:Z").FormatConditions.Add Type:=xlExpression, Formula1:= _
                        '"=NOT(ISBLANK(Z1))"
                        'xlSheet.Range("Z:Z").FormatConditions(Selection.FormatConditions.Count).SetFirstPriority
                        'With xlSheet.Range("Z:Z").FormatConditions(1).Interior
                        '    .PatternColorIndex = 0
                        '    .Color = 65535
                        '    .TintAndShade = 0
            ...
            Select Case mySheetName
                Case "Photo Review"
                    ...
                    If hasPhotoReviewWorksheet Then
                        ...
                        xlBook.Worksheets("Photo Review").select
                        xlSheet.columns("G:G").EntireColumn.hidden = True
                        xlSheet.columns("H:I").EntireColumn.hidden = False
                ...
                Case "Verification Report"
                    ...
                    'Post Sheet Formatting 
                    '2/16/2022 add the omitted highlight cell rule to the verification report sheet
                    set highlightrange = xlSheet.range("Z12:Z2700")
                    highlightrange.FormatConditions.Add Type:=xlExpression, Formula1:= _
                        "=NOT(ISBLANK(Z12))"
                    highlightrange.FormatConditions(highlightrange.FormatConditions.Count).SetFirstPriority
                    With highlightrange.FormatConditions(1).Interior
                        .PatternColorIndex = 0
                        .Color = 65535
                        .TintAndShade = 0
                        .PatternTintAndShade = 0
